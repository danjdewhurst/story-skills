...
1. Clarify the pass type unless the user already specified it:
   ...
   - **Copyedit** - distinct from proof/polish: enforce a style baseline (hyphenation, capitalization, naming, numbers) and continuity of surface detail (hair color, room layouts, name spellings). This pass is mechanical consistency, not prose quality — prose quality belongs to the line edit. Read: the project style note (create one if missing), `glossary/`, character and location files. Update: chapters, `glossary/`, character files where details drifted.
