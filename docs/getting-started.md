...
| Pin a global install to one release | `npm install -g story-skills@0.11.0` |
| Run one release without installing | `npx story-skills@0.11.0 --help` |
| Remove a global install | `npm uninstall -g story-skills` |

Pass `@latest` or a version to `npx` when you need to be sure which release runs, because `npx` can reuse a copy it downloaded earlier. To pin the skills for a manual copy, run `git checkout v0.11.0` in the clone before you copy the folders.
