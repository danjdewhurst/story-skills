...
|---|---|---|
| npm, pinned | `npx --yes --package story-skills@0.11.0 story <command>` | Fetches the published package. |
| npm, installed once per job | `npm install -g story-skills@0.11.0`, then `story <command>` | Faster when a job runs several commands. |
| GitHub tag | `npx --yes --package github:danjdewhurst/story-skills#v0.11.0 story <command>` | What the templates use. Fetches the tagged release from GitHub, because the templates predate the npm package. |
...
```shell
npx --yes --package story-skills@0.11.0 story --version
```
...
The hook assumes `story` is on your `PATH`, for example after `npm install -g story-skills`. If it is not, replace `story` with `npx --yes story-skills@0.11.0` or with `node <skills-dir>/story-maintenance/scripts/story.js`. Warnings do not block the commit.
...
```shell
npm install -g story-skills@0.11.0
story validate .
story links .
story continuity .
```
